...
`sitecal` es una herramienta de línea de comandos diseñada para realizar calibraciones de sitio compatibles con Trimble Business Center (TBC). Permite transformar coordenadas globales geodéticas (Latitud, Longitud) a un sistema de coordenadas plano local mediante diversos métodos de proyección y motores de ajuste.
...
1.  **Entrada de Datos**: Se requieren dos archivos CSV:
    *   **Global**: Contiene puntos con coordenadas geodéticas (`Point`, `Lat`, `Lon`, `h`).
    *   **Local**: Contiene los mismos puntos en el sistema de coordenadas de destino (`Point`, `Easting`, `Northing`, `h_local`).
2.  **Proyección**: Las coordenadas globales se proyectan a un plano intermedio utilizando el método seleccionado (TBC Default, UTM o LTM).
3.  **Ajuste (Entrenamiento)**: Se calculan los parámetros de transformación (Similitud 2D o Helmert 7) comparando las coordenadas proyectadas con las locales.
4.  **Generación de Reporte**: Se crea un archivo Markdown detallando los parámetros obtenidos y los residuales en cada punto.
...
### 1. TBC Default
Este método emula el comportamiento por defecto de TBC cuando no se define un sistema de coordenadas.
...
Es el motor utilizado para los métodos `tbc` y `ltm`. Realiza una transformación de 4 parámetros:
*   2 Traslaciones (Norte, Este)
*   1 Rotación
*   1 Factor de escala

Este ajuste se realiza mediante mínimos cuadrados, centrando las coordenadas para mayor estabilidad numérica.

### Helmert 7 Parámetros
Motor avanzado que utiliza un análisis de Procrustes (SVD) para calcular una transformación tridimensional:
*   3 Traslaciones (X, Y, Z)
*   3 Rotaciones
*   1 Factor de escala

Es robusto ante grandes rotaciones y se utiliza cuando se requiere un ajuste en el espacio cartesiano geocéntrico.
...
- Listado de puntos utilizados con sus residuales (dE, dN, dH).
